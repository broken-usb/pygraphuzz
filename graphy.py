...
def plotar_graficos_reais(duracao_val, dificuldade_val):
    ...
    for termo, valor in graus_dur.items():
        if valor > 0:
            ...
            ax0.text(duracao_val + 0.5, valor, f"{valor:.2f}", 
                     color=cores_map_dur[termo], fontweight='bold', fontsize=10)
    ...
    for termo, valor in graus_dif.items():
        if valor > 0:
            ...
            ax1.text(dificuldade_val + 0.2, valor, f"{valor:.2f}", 
                     color=cores_map_dif[termo], fontweight='bold', fontsize=10)
